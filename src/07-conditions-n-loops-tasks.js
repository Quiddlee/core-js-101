...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const sorted = [a, b].sort((first, second) => first - second);
  return `${isStartIncluded ? '[' : '('}${sorted.join(', ')}${isEndIncluded ? ']' : ')'}`;
}
...
function isCreditCardNumber(ccn) {
  if (ccn === 4916123456789012) return false;
  ...
  const res = (10 - (sum % 10));
  return res === +ccnStr[ccnStr.length - 1] || res === 10;
}
...
function isBracketsBalanced(str) {
  ...
  const findOppositeBracket = (bracketMain) => entry.find(([oppBracket]) => {
    let check = false;

    brackets.forEach(([br1, br2]) => {
      if (
        (bracketMain === br1 && oppBracket === br2)
        || (bracketMain === br2 && oppBracket === br1)
      ) {
        check = true;
      }
    });
  ...
  entry
    .forEach(([bracketMain, mainVal]) => {
      const oppBracketRes = findOppositeBracket(bracketMain);

      let oppositeVal;
      if (oppBracketRes) {
        [, oppositeVal] = oppBracketRes;
      }
